...
jQuery.event = {
	...
	// 每次调用 on 绑定事件的时候都会添加向对应的事件
	// 实际向DOM注册回调的时候，都是向它对应的数据缓存中的event对象绑定一个回调函数
	add: function( elem, types, handler, data, selector ) {

		var handleObjIn, eventHandle, tmp,
			events, t, handleObj,
			special, handlers, type, namespaces, origType,
			elemData = dataPriv.get( elem );		// 每次实际上都是向缓存模块中添加处理函数
		...
		if ( !(events = elemData.events) ) {	// elemData表示当前的DOM所对应的私有缓存，如果不存在events这个属性就添加一个空对象
	...
	// 分派事件，真正的触发函数在这里会根据selector再进行一遍过滤
